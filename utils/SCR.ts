  ...
  for (let i = 3; i < lines.length; i++) {
    ...
    if (line.startsWith("N")) {
      ...
      if (flightParts.length >= 6) {
        ...
        const flightNumber = flightType === 'Departure' ? flightParts[1] : flightParts[0].replace("N", "").trim();

        // Parse the remaining parts correctly based on flight type
        const [startDate, endDate] = flightType === 'Departure'
        ...
        const serviceInfo = flightParts.slice(6).join(" ").replace("/", "").trim();
