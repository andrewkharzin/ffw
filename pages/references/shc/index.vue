<template>
  <div class="p-4">
    ...
    <div v-if="!loading && !error" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      <div v-for="dgrClass in dgrClasses" :key="dgrClass.id" class="p-4 border rounded-lg shadow">
        ...
        <p><strong>Emergency Actions:</strong> {{ dgrClass.emergency_actions }}</p>
        <a v-if="dgrClass.label_link" :href="dgrClass.label_link" class="text-blue-500 underline" target="_blank">Label Link</a>
      </div>
    </div>
  </div>
</template>
